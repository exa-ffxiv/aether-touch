...
using static FFXIVClientStructs.FFXIV.Client.Sound.ISoundData.Delegates;
...
namespace AetherTouch.UI.Tabs
{
    public class PatternTab
    {
        ...
        public void DrawPatternList()
        {
            ...
            if (ImGui.BeginListBox("##PatternList", new Vector2(200f, ImGui.GetWindowHeight() - 85)))
            {
        ...
        public void DrawPatternDetail()
        {
            if (selectedPatternIndex > -1 && selectedPatternIndex < dataManager.Patterns.Count)
            {
                ...
                // Begin Scrollable area for steps
                ...
                {
                    DrawStepDetail(selected, i);
                }

                // End Scrollable area for steps
                ImGui.EndChild();
        ...
        public void DrawStepDetail(Pattern selected, int i)
        {
            ...
            ImGui.BeginGroup();
            DrawStepDetailTopRow(selected, step, i, ref duration);
            DrawStepDetailBottomRow(selected, step, i, ref intensity);
            ImGui.EndGroup();

            DrawPatternDetailBorder(minPos);
            // Add some spacing between the groups
            ImGui.SetCursorScreenPos(ImGui.GetCursorScreenPos() + new Vector2(0.0f, 6.0f));
        }

        private void DrawStepDetailTopRow(Pattern selected, Step step, int i, ref int duration)
        {
            ...
            if (ImGui.InputInt($"Duration (Milliseconds)##{i}", ref duration, 50, 250))
            {
        ...
        private void DrawStepDetailBottomRow(Pattern selected, Step step, int i, ref int intensity)
        {
        ...
        private void DrawPatternDetailBorder(Vector2 minPos)
        {
